...
#include "HashFunc.h"
#include "DbVrfy.h"
#include "DbVrfyutil.h"
#include "MpFget.h"
#include "MpFput.h"
#include "DbOvflVrfy.h"
#include "DbVrfy.h"
#include "DbRet.h"
#include "BTreeVerify.h"
...
{
public:
#if !defined(HAVE_HASH) || !defined(HAVE_VERIFY)
	static int ham_vrfy_meta(DB *dbp, VRFY_DBINFO *vdp, HMETA *m, db_pgno_t pgno, u_int32_t flags)
	{
		COMPQUIET(vdp, NULL);
		COMPQUIET(m, NULL);
		COMPQUIET(pgno, 0);
	...
	static int ham_vrfy(DB *dbp, VRFY_DBINFO *vdp, PAGE *h, db_pgno_t pgno, u_int32_t flags)
	{
		COMPQUIET(vdp, NULL);
		COMPQUIET(h, NULL);
		COMPQUIET(pgno, 0);
	...
	static int ham_vrfy_item(DB *dbp, VRFY_DBINFO *vdp, db_pgno_t pgno, PAGE *h, u_int32_t i, u_int32_t flags);
	static int ham_vrfy_structure(DB *dbp, VRFY_DBINFO *vdp, db_pgno_t meta_pgno, u_int32_t flags)
	{
		COMPQUIET(vdp, NULL);
	...
	static int ham_vrfy_bucket(DB *dbp, VRFY_DBINFO *vdp, HMETA *m, u_int32_t bucket, u_int32_t flags);
	static int ham_vrfy_hashing(DB *dbp, u_int32_t nentries, HMETA *m, u_int32_t thisbucket, db_pgno_t pgno, u_int32_t flags, u_int32_t (*hfunc) __P((DB *, const void *, u_int32_t)))
	{
	...
	static int ham_salvage(DB *dbp, VRFY_DBINFO *vdp, db_pgno_t pgno, PAGE *h, void *handle, int (*callback) __P((void *, const void *)), u_int32_t flags)
	{
		COMPQUIET(vdp, NULL);
		COMPQUIET(pgno, 0);
		COMPQUIET(h, NULL);
	...
	static int ham_meta2pgset(DB *dbp, VRFY_DBINFO *vdp, HMETA *hmeta, u_int32_t flags, DB *pgset)
	{
		COMPQUIET(vdp, NULL);
	...
	static int ham_dups_unsorted(DB *dbp, u_int8_t *buf, u_int32_t len);
	...
	*/
	int CHashVerify::ham_salvage(DB *dbp, VRFY_DBINFO *vdp, db_pgno_t pgno, PAGE *h, void *handle, int (*callback) __P((void *, const void *)), u_int32_t flags)
	{
		DBT dbt, unkdbt;
		db_pgno_t dpgno;
		...
		*/
		if ((ret = COsAlloc::os_malloc(dbp->dbenv, dbp->pgsize, &buf)) != 0)
			return (ret);
		...
		for (i = 0;; i++) {
			...
			ret = CDbVrfy::db_vrfy_inpitem(dbp,
...
keydata:			memcpy(buf, HKEYDATA_DATA(hk), len);
	dbt.size = len;
	dbt.data = buf;
	if ((ret = CDbVrfyutil::db_vrfy_prdbt(&dbt,
...
case H_OFFPAGE:
	...
	if ((ret = CDbOvflVrfy::db_safe_goff(dbp, vdp,
		dpgno, &dbt, &buf, flags)) != 0) {
			err_ret = ret;
			(void)CDbVrfyutil::db_vrfy_prdbt(&unkdbt, 0, " ",
...
case H_OFFDUP:
	...
	if (!IS_VALID_PGNO(dpgno) || (i % 2 == 0)) {
		if ((ret =
			CDbVrfyutil::db_vrfy_prdbt(&unkdbt, 0, " ",
			handle, callback, 0, vdp)) != 0)
			err_ret = ret;
	} else if ((ret = CDbVrfy::db_salvage_duptree(dbp,
...
case H_DUPLICATE:
	...
	*/
	...
	for (tlen = 0; tlen + sizeof(db_indx_t) < len;
		tlen += dlen) {
			...
			*/
			...
			dbt.size = dlen;
			dbt.data = buf;
			if ((ret = CDbVrfyutil::db_vrfy_prdbt(&dbt, 0, " ",
				handle, callback, 0, vdp)) != 0)
				err_ret = ret;
			tlen += sizeof(db_indx_t);
	}
	break;
		...
		COsAlloc::os_free(dbp->dbenv, buf);
		if ((t_ret = CDbVrfyutil::db_salvage_markdone(vdp, pgno)) != 0)
			return (t_ret);
	...
	*/
	int CHashVerify::ham_vrfy_hashing(DB *dbp, u_int32_t nentries, HMETA *m, u_int32_t thisbucket, db_pgno_t pgno, u_int32_t flags, u_int32_t (*hfunc) __P((DB *, const void *, u_int32_t)))
	{
		DBT dbt;
		DB_MPOOLFILE *mpf;
		PAGE *h;
		...
		for (i = 0; i < nentries; i += 2) {
			...
			*/
			if ((ret = CDbRet::db_ret(dbp, h, i, &dbt, NULL, NULL)) != 0)
				goto err;
...
err:	if (dbt.data != NULL)
			COsAlloc::os_ufree(dbp->dbenv, dbt.data);
		if ((t_ret = CMpFput::memp_fput(mpf, h, 0)) != 0)
			return (t_ret);
	...
	* PUBLIC:     u_int32_t));
	*/
	int CHashVerify::ham_vrfy_structure(DB *dbp, VRFY_DBINFO *vdp, db_pgno_t meta_pgno, u_int32_t flags)
	{
		DB *pgset;
		DB_MPOOLFILE *mpf;
		HMETA *m;
		...
		if ((ret = CDbVrfyutil::db_vrfy_pgset_get(pgset, meta_pgno, &p)) != 0)
			return (ret);
		if (p != 0) {
			EPRINT((dbp->dbenv,
		...
		if ((ret = CDbVrfyutil::db_vrfy_pgset_inc(pgset, meta_pgno)) != 0)
			return (ret);

		/* Get the meta page;  we'll need it frequently. */
		if ((ret = CMpFget::memp_fget(mpf, &meta_pgno, 0, &m)) != 0)
			return (ret);

		/* Loop through bucket by bucket. */
		for (bucket = 0; bucket <= m->max_bucket; bucket++)
			if ((ret =
				ham_vrfy_bucket(dbp, vdp, m, bucket, flags)) != 0) {
			...
			*/
			for (bucket = m->max_bucket + 1; spares_entry = CDbLog2::db_log2(bucket + 1),
				spares_entry < NCACHED && m->spares[spares_entry] != 0; bucket++) {
					pgno = BS_TO_PAGE(bucket, m->spares);
					if ((ret = CDbVrfyutil::db_vrfy_getpageinfo(vdp, pgno, &pip)) != 0)
						goto err;
					...
					if (pip->type == P_INVALID) {
						if ((ret = CDbVrfyutil::db_vrfy_putpageinfo(dbp->dbenv,
							vdp, pip)) != 0)
							goto err;
						continue;
					}
					...
					if (pip->type != P_HASH) {
						...
						if ((ret = CDbVrfyutil::db_vrfy_pgset_get(pgset, pgno, &p)) != 0)
							goto err;
						if (p != 0) {
							EPRINT((dbp->dbenv,
							...
							if ((ret =
								CDbVrfyutil::db_vrfy_pgset_inc(pgset, pgno)) != 0)
								goto err;
							if ((ret = CDbVrfyutil::db_vrfy_putpageinfo(dbp->dbenv,
								vdp, pip)) != 0)
								goto err;
					...
					(void)CDbVrfyutil::db_vrfy_putpageinfo(dbp->dbenv, vdp, pip);
					goto err;
			}

err:	if ((t_ret = CMpFput::memp_fput(mpf, m, 0)) != 0)
			return (t_ret);
		if (h != NULL && (t_ret = CMpFput::memp_fput(mpf, h, 0)) != 0)
			return (t_ret);
	...
	*/
	int CHashVerify::ham_vrfy_meta(DB *dbp, VRFY_DBINFO *vdp, HMETA *m, db_pgno_t pgno, u_int32_t flags)
	{
		HASH *hashp;
		VRFY_PAGEINFO *pip;
		...
		*/
		if (!F_ISSET(pip, VRFY_INCOMPLETE) &&
			(ret = CDbVrfy::db_vrfy_meta(dbp, vdp, &m->dbmeta, pgno, flags)) != 0) {
				if (ret == DB_VERIFY_BAD)
					isbad = 1;
				else
					goto err;
		...
		if (!LF_ISSET(DB_NOORDERCHK))
			...
			*/
			pwr = (m->max_bucket == 0) ? 1 : 1 << CDbLog2::db_log2(m->max_bucket + 1);
...
err:	if ((t_ret =
			CDbVrfyutil::db_vrfy_putpageinfo(dbp->dbenv, vdp, pip)) != 0 && ret == 0)
			ret = t_ret;
		if (LF_ISSET(DB_SALVAGE) &&
			(t_ret = CDbVrfyutil::db_salvage_markdone(vdp, pgno)) != 0 && ret == 0)
			ret = t_ret;
		return ((ret == 0 && isbad == 1) ? DB_VERIFY_BAD : ret);
	}
	...
	* PUBLIC:     u_int32_t));
	*/
	int CHashVerify::ham_vrfy(DB *dbp, VRFY_DBINFO *vdp, PAGE *h, db_pgno_t pgno, u_int32_t flags)
	{
		VRFY_PAGEINFO *pip;
		...
		if ((ret = CDbVrfy::db_vrfy_datapage(dbp, vdp, h, pgno, flags)) != 0) {
			if (ret == DB_VERIFY_BAD)
				isbad = 1;
			else
				goto err;
...
err:	if ((t_ret =
	...
	* PUBLIC:     DB *));
	*/
	int CHashVerify::ham_meta2pgset(DB *dbp, VRFY_DBINFO *vdp, HMETA *hmeta, u_int32_t flags, DB *pgset)
	{
		DB_MPOOLFILE *mpf;
		PAGE *h;
		db_pgno_t pgno;
		...
		for (bucket = 0; bucket <= hmeta->max_bucket; bucket++) {
			...
			*/
			for (;;) {
				...
				if ((ret = CMpFput::memp_fput(mpf, h, 0)) != 0)
					return (ret);
				...
				*/
				if ((ret = CDbVrfyutil::db_vrfy_pgset_get(pgset, pgno, &val)) != 0)
					return (ret);
				if (val != 0)
					break;
			}
	...
	*/
	int CHashVerify::ham_vrfy_item(DB *dbp, VRFY_DBINFO *vdp, db_pgno_t pgno, PAGE *h, u_int32_t i, u_int32_t flags)
	{
		HOFFPAGE hop;
		HOFFDUP hod;
...
case H_DUPLICATE:
	...
	*/
	...
	F_SET(pip, VRFY_HAS_DUPS);
	if (!LF_ISSET(DB_NOORDERCHK) &&
		ham_dups_unsorted(dbp, databuf, len))
...
case H_OFFPAGE:
	...
	if ((ret = CDbVrfyutil::db_vrfy_childput(vdp, pip->pgno, &child)) != 0)
		goto err;
	break;
case H_OFFDUP:
...
err:	if ((t_ret =
	...
	*/
	int CHashVerify::ham_vrfy_bucket(DB *dbp, VRFY_DBINFO *vdp, HMETA *m, u_int32_t bucket, u_int32_t flags)
	{
		HASH *hashp;
		VRFY_CHILDINFO *child;
		...
		if ((ret = CDbVrfyutil::db_vrfy_getpageinfo(vdp, PGNO(m), &mip)) != 0)
			return (ret);
		...
		*/
		...
		for (;;) {
			...
			if (!LF_ISSET(DB_SALVAGE))
				CDbVrfy::db_vrfy_struct_feedback(dbp, vdp);

			if ((ret = CDbVrfyutil::db_vrfy_pgset_get(vdp->pgset, pgno, &p)) != 0)
				goto err;
			if (p != 0) {
				EPRINT((dbp->dbenv,
				...
				/* Unsafe to continue. */
				goto err;
			} else if ((ret = CDbVrfyutil::db_vrfy_pgset_inc(vdp->pgset, pgno)) != 0)
				goto err;
			...
			*/
			...
			if ((ret = CDbVrfyutil::db_vrfy_childcursor(vdp, &cc)) != 0)
				goto err;
			for (ret = CDbVrfyutil::db_vrfy_ccset(cc, pip->pgno, &child); ret == 0;
				ret = CDbVrfyutil::db_vrfy_ccnext(cc, &child))
				if (child->type == V_OVERFLOW) {
					if ((ret = CDbOvflVrfy::db_vrfy_ovfl_structure(dbp, vdp,
					...
					if ((ret = CDbVrfy::db_vrfy_duptype(dbp,
						vdp, child->pgno, flags)) != 0) {
							isbad = 1;
							continue;
					}
#ifdef HAVE_BTREE
					if ((ret = CBTreeVerify::bam_vrfy_subtree(dbp, vdp,
				...
				ret = CDbVrfyutil::db_vrfy_putpageinfo(dbp->dbenv, vdp, pip);
				...
				if ((ret = CDbVrfyutil::db_vrfy_getpageinfo(vdp, next_pgno, &pip)) != 0)
					goto err;
...
err:	if (cc != NULL && ((t_ret = CDbVrfyutil::db_vrfy_ccclose(cc)) != 0) && ret == 0)
			ret = t_ret;
		if (mip != NULL && ((t_ret =
			CDbVrfyutil::db_vrfy_putpageinfo(dbp->dbenv, vdp, mip)) != 0) && ret == 0)
			ret = t_ret;
		if (pip != NULL && ((t_ret =
			CDbVrfyutil::db_vrfy_putpageinfo(dbp->dbenv, vdp, pip)) != 0) && ret == 0)
			ret = t_ret;
	...
	*	0 if there are not.
	*/
	int CHashVerify::ham_dups_unsorted(DB *dbp, u_int8_t *buf, u_int32_t len)
	{
		DBT a, b;
		...
		func = (dbp->dup_compare == NULL) ? CBTreeCompare::bam_defcmp : dbp->dup_compare;
