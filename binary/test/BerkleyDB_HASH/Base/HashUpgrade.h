...
public:
	...
	static int ham_30_hashmeta(DB *dbp, char *real_name, u_int8_t *obuf)
	{
	...
	static int ham_30_sizefix(DB *dbp, DB_FH *fhp, char *realname, u_int8_t *metabuf)
	{
		COMPQUIET(fhp, NULL);
	...
	static int ham_31_hashmeta(DB *dbp, char *real_name, u_int32_t flags, DB_FH *fhp, PAGE *h, int *dirtyp)
	{
		COMPQUIET(real_name, NULL);
		COMPQUIET(flags, 0);
		COMPQUIET(fhp, NULL);
	...
	static int ham_31_hash(DB *dbp, char *real_name, u_int32_t flags, DB_FH *fhp, PAGE *h, int *dirtyp)
	{
		COMPQUIET(real_name, NULL);
		COMPQUIET(flags, 0);
		COMPQUIET(fhp, NULL);
	...
	*/
	static int ham_30_hashmeta(DB *dbp, char *real_name, u_int8_t *obuf)
	{
		...
		*/
		...
		max_entry = CDbLog2::db_log2(maxb + 1);   /* highest spares entry in use */
		n_spares[0] = 1;
		for (i = 1; i < NCACHED && i <= max_entry; i++)
			n_spares[i] = 1 + o_spares[i - 1];

		/* Replace the unique ID. */
		if ((ret = COsFid::os_fileid(dbenv, real_name, 1, newmeta.dbmeta.uid)) != 0)
			return (ret);
	...
	*/
	...
	static int ham_30_sizefix(DB *dbp, DB_FH *fhp, char *realname, u_int8_t *metabuf)
	{
		...
		*/
		dbp->pgsize = pagesize;
		if ((ret = CDbUpg::db_lastpgno(dbp, realname, fhp, &last_actual)) != 0)
			return (ret);
		...
		*/
		if (last_desired > last_actual) {
			if ((ret = COsSeek::os_seek(dbenv,
				fhp, pagesize, last_desired, 0, 0, DB_OS_SEEK_SET)) != 0)
				return (ret);
			if ((ret = COsRw::os_write(dbenv, fhp, buf, pagesize, &nw)) != 0)
				return (ret);
		}
...
#ifdef HAVE_BTREE	//ohne btree keine offpagedups
			if (HPAGE_PTYPE(hk) == H_OFFDUP) {
				memcpy(&pgno, HOFFDUP_PGNO(hk), sizeof(db_pgno_t));
				tpgno = pgno;
				if ((ret = CDbUpgOpd::db_31_offdup(dbp, real_name, fhp,
