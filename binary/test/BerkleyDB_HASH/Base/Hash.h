...
public:
	...
	*/
	...
	static int ham_quick_delete(DBC *dbc)
	{
		return (db_no_hash_am(dbc->dbp->dbenv));
	}


	static int ham_c_count(DBC *dbc, db_recno_t *recnop)
	{
		COMPQUIET(recnop, NULL);
		return (db_no_hash_am(dbc->dbp->dbenv));
	}

	static int ham_c_dup(DBC *orig_dbc, DBC *new_dbc)
	{
		COMPQUIET(new_dbc, NULL);
		return (db_no_hash_am(orig_dbc->dbp->dbenv));
	}


	static int ham_c_init(DBC *dbc)
	{
		return (db_no_hash_am(dbc->dbp->dbenv));
	}

	static int ham_c_close(DBC *dbc, db_pgno_t root_pgno, int *rmroot);
	static int ham_c_destroy(DBC *dbc);
	static int ham_c_del(DBC *dbc);
	static int ham_c_get(DBC *dbc, DBT *key, DBT *data, u_int32_t flags, db_pgno_t *pgnop);
	static int ham_bulk(DBC *dbc, DBT *data, u_int32_t flags);
	static int ham_c_put(DBC *dbc, DBT *key, DBT *data, u_int32_t flags, db_pgno_t *pgnop);
	static int ham_expand_table(DBC *dbc);
	static u_int32_t ham_call_hash(DBC *dbc, u_int8_t *k, u_int32_t len);
	static int ham_dup_return(DBC *dbc, DBT *val, u_int32_t flags);
	static int ham_overwrite(DBC *dbc, DBT *nval, u_int32_t flags);
	static int ham_lookup(DBC *dbc, const DBT *key, u_int32_t sought, db_lockmode_t mode, db_pgno_t *pgnop);
	static int ham_init_dbt(DB_ENV *dbenv, DBT *dbt, u_int32_t size, void **bufp, u_int32_t *sizep);
	static int ham_c_update(DBC *dbc, u_int32_t len, int add, int is_dup);
	static int ham_get_clist(DB *dbp, db_pgno_t pgno, u_int32_t indx, DBC ***listp);
	static int ham_c_writelock(DBC *dbc);
	...
	*/
	static int ham_quick_delete(DBC *dbc)
	{
		...
		*/
		...
		if ((ret = ham_c_writelock(dbc)) == 0)
			ret = CHashPage::ham_del_pair(dbc, 1);
	...
	*/
	static int ham_c_count(DBC *dbc, db_recno_t *recnop)
	{
		...
		if ((ret = CHashPage::ham_get_cpage(dbc, DB_LOCK_READ)) != 0)
			return (ret);
		...
		case H_DUPLICATE:
			...
			break;
		default:
			ret = CDbErr::db_pgfmt(dbp->dbenv, hcp->pgno);
			goto err;
		}

		*recnop = recno;

	err:	if ((t_ret = CMpFput::memp_fput(mpf, hcp->page, 0)) != 0 && ret == 0)
				ret = t_ret;
			hcp->page = NULL;
			return (ret);
	}
	...
	*/
	static int ham_c_dup(DBC *orig_dbc, DBC *new_dbc)
	{
		HASH_CURSOR *orig, *new__renamed;
		int ret;

		orig = (HASH_CURSOR *)orig_dbc->internal;
		new__renamed = (HASH_CURSOR *)new_dbc->internal;

		new__renamed->bucket = orig->bucket;
		new__renamed->lbucket = orig->lbucket;
		new__renamed->dup_off = orig->dup_off;
		new__renamed->dup_len = orig->dup_len;
		new__renamed->dup_tlen = orig->dup_tlen;

		if (F_ISSET(orig, H_DELETED))
			F_SET(new__renamed, H_DELETED);
		if (F_ISSET(orig, H_ISDUP))
			F_SET(new__renamed, H_ISDUP);
		...
		*/
		if (orig_dbc->txn == NULL && LOCK_ISSET(orig->lock))
			if ((ret = CHashPage::ham_lock_bucket(new_dbc, DB_LOCK_READ)) != 0)
				return (ret);
	...
	static int ham_c_init(DBC *dbc)
	{
		...
		dbenv = dbc->dbp->dbenv;
		if ((ret = COsAlloc::os_calloc(dbenv,
			1, sizeof(struct cursor_t), &new_curs)) != 0)
			return (ret);
		if ((ret = COsAlloc::os_malloc(dbenv,
			dbc->dbp->pgsize, &new_curs->split_buf)) != 0) {
			COsAlloc::os_free(dbenv, new_curs);
			return (ret);
		}
		...
		dbc->c_am_bulk = ham_bulk;
		dbc->c_am_close = ham_c_close;
		dbc->c_am_del = ham_c_del;
		dbc->c_am_destroy = ham_c_destroy;
		dbc->c_am_get = ham_c_get;
		dbc->c_am_put = ham_c_put;
		dbc->c_am_writelock = ham_c_writelock;

		return (CHashPage::ham_item_init(dbc));
	}
...
#ifdef HAVE_BTREE
	// hash without btree has no off page dups
		if (dbc->internal->opd != NULL) {
			...
			if (F_ISSET(dbc->dbp, DB_AM_READ_UNCOMMITTED) &&
				 F_ISSET((CBTreeCursor *)
				 dbc->internal->opd->internal, C_DELETED))
				lock_mode = DB_LOCK_WRITE;

			if ((ret = CHashPage::ham_get_cpage(dbc, lock_mode)) != 0)
				goto out;
			...
			if ((ret =
				...
				goto out;
			if (doroot != 0) {
				if ((ret = CHashPage::ham_del_pair(dbc, 1)) != 0)
					goto out;
	...
	out:	if (hcp->page != NULL && (t_ret =
			CMpFput::memp_fput(mpf, hcp->page, dirty)) != 0 && ret == 0)
			ret = t_ret;
		if (gotmeta != 0 && (t_ret = CHashMeta::ham_release_meta(dbc)) != 0 && ret == 0)
			ret = t_ret;

	done:	if ((t_ret = CHashPage::ham_item_init(dbc)) != 0 && ret == 0)
			ret = t_ret;
		return (ret);
	}
	...
	static int ham_c_destroy(DBC *dbc)
	{
		...
		if (hcp->split_buf != NULL)
			COsAlloc::os_free(dbc->dbp->dbenv, hcp->split_buf);
		COsAlloc::os_free(dbc->dbp->dbenv, hcp);
	...
	static int ham_c_del(DBC *dbc)
	{
		...
		if ((ret = CHashPage::ham_get_cpage(dbc, DB_LOCK_WRITE)) != 0)
			goto out;
		...
		if (F_ISSET(hcp, H_ISDUP)) { /* On-page duplicate. */
			if (hcp->dup_off == 0 &&
				...
				repldbt.data = HKEYDATA_DATA(H_PAIRDATA(dbp, hcp->page,
					hcp->indx));
				if ((ret = CHashPage::ham_replpair(dbc, &repldbt, 0)) == 0) {
					hcp->dup_tlen -= DUP_SIZE(hcp->dup_len);
					F_SET(hcp, H_DELETED);
					ret = ham_c_update(dbc,
	...
	out:	if (hcp->page != NULL) {
			if ((t_ret = CMpFput::memp_fput(mpf,
	...
	static int ham_c_get(DBC *dbc, DBT *key, DBT *data, u_int32_t flags, db_pgno_t *pgnop)
	{
		...
		case DB_PREV:
			if (IS_INITIALIZED(dbc)) {
				ret = CHashPage::ham_item_prev(dbc, lock_type, pgnop);
				break;
			}
			/* FALLTHROUGH */
		case DB_LAST:
			ret = CHashPage::ham_item_last(dbc, lock_type, pgnop);
			break;
		case DB_NEXT_NODUP:
			F_SET(hcp, H_NEXT_NODUP);
			/* FALLTHROUGH */
		case DB_NEXT:
			if (IS_INITIALIZED(dbc)) {
				ret = CHashPage::ham_item_next(dbc, lock_type, pgnop);
				break;
			}
			/* FALLTHROUGH */
		case DB_FIRST:
			ret = CHashPage::ham_item_first(dbc, lock_type, pgnop);
			break;
		case DB_NEXT_DUP:
		...
		case DB_GET_BOTH_RANGE:
			ret = ham_lookup(dbc, key, 0, lock_type, pgnop);
			get_key = 0;
			break;
		case DB_GET_BOTHC:
			F_SET(hcp, H_DUPONLY);
		...
		case DB_CURRENT:
			...
			ret = CHashPage::ham_item(dbc, lock_type, pgnop);
			break;
		default:
			ret = CDbErr::db_unknown_flag(dbp->dbenv, "__ham_c_get", flags);
			break;
		}
		...
		for (;;) {
			if (ret != 0 && ret != DB_NOTFOUND)
				...
				if (*pgnop == PGNO_INVALID)
					ret = ham_dup_return(dbc, data, flags);
				break;
			...
			switch (flags) {
				...
				case DB_PREV:
				case DB_PREV_NODUP:
					ret = CMpFput::memp_fput(mpf, hcp->page, 0);
					hcp->page = NULL;
					if (hcp->bucket == 0) {
						ret = DB_NOTFOUND;
					...
					if (ret == 0)
						ret = CHashPage::ham_item_prev(dbc,
							lock_type, pgnop);
					break;
				case DB_FIRST:
				case DB_NEXT:
				case DB_NEXT_NODUP:
					...
					if (ret == 0)
						ret = CHashPage::ham_item_next(dbc,
							lock_type, pgnop);
					break;
				case DB_GET_BOTH:
				case DB_GET_BOTHC:
	...
	err:	if ((t_ret = CHashMeta::ham_release_meta(dbc)) != 0 && ret == 0)
			ret = t_ret;
	...
	static int ham_bulk(DBC *dbc, DBT *data, u_int32_t flags)
	{
		...
		dbuf = static_cast<u_int8_t*>(data->data);
		np = dp = dbuf;
		...
		do {
			if (is_key) {
				...
				if (HPAGE_PTYPE(hk) == H_OFFPAGE) {
					...
					if (key_size > space)
						goto get_key_space;
					if ((ret = dbc->bulk_overflow(key_size, pgno, np)) != 0)
						return (ret);
					space -= key_size;
	...
	back_up:
						...
						if ((ret = CHashPage::ham_item_prev(dbc,
							lock_mode, &pgno)) != 0) {
							if (ret != DB_NOTFOUND)
								return (ret);
							if ((ret = CMpFput::memp_fput(mpf,
								cp->page, 0)) != 0)
								return (ret);
							cp->page = NULL;
							if (cp->bucket == 0) {
							...
							cp->pgno = BUCKET_TO_PAGE(cp,
								cp->bucket);
							cp->indx = NDX_INVALID;
							if ((ret = CHashMeta::ham_release_meta(
								dbc)) != 0)
								return (ret);
							if ((ret = CHashPage::ham_item_prev(dbc,
								lock_mode, &pgno)) != 0)
	...
	get_space:
			...
			case H_OFFDUP:
				...
				saveoff = offp;
				if ((ret = CBTreeCursor::bam_bulk_duplicates(dbc,
			...
			case H_OFFPAGE:
				...
				if ((ret = dbc->bulk_overflow(size, pgno, np)) != 0)
					return (ret);
		...
		if (ret == 0 && next_key && indx >= NUM_ENT(pg)) {
			if ((ret = CHashPage::ham_item_next(dbc, lock_mode, &pgno)) == 0)
				goto next_pg;
			if (ret != DB_NOTFOUND)
				return (ret);
			if ((ret = CMpFput::memp_fput(dbc->dbp->mpf, cp->page, 0)) != 0)
				return (ret);
			cp->page = NULL;
			if ((ret = CHashMeta::ham_get_meta(dbc)) != 0)
				return (ret);
			...
			if (cp->bucket > cp->hdr->max_bucket) {
				...
				cp->indx = NDX_INVALID;
				F_CLR(cp, H_ISDUP);
				ret = CHashPage::ham_item_next(dbc, lock_mode, &pgno);
			}

			if ((t_ret = CHashMeta::ham_release_meta(dbc)) != 0)
				return (t_ret);
			if (ret == 0)
				goto next_pg;
			if (ret != DB_NOTFOUND)
	...
	static int ham_c_put(DBC *dbc, DBT *key, DBT *data, u_int32_t flags, db_pgno_t *pgnop)
	{
		...
		case DB_NODUPDATA:
			...
			if ((ret = ham_lookup(dbc,
				...
				if (F_ISSET(data, DB_DBT_PARTIAL) && data->doff != 0) {
					 ...
					 * val.
					 */
					if ((ret = ham_init_dbt(dbp->dbenv, &tmp_val,
						data->size + data->doff,
						&dbc->my_rdata.data,
						&dbc->my_rdata.ulen)) == 0) {
				...
				if (ret == 0)
					ret = CHashPage::ham_add_el(dbc, key, myval, H_KEYDATA);
				goto done;
			}
			break;
		case DB_BEFORE:
		case DB_AFTER:
		case DB_CURRENT:
			ret = CHashPage::ham_item(dbc, DB_LOCK_WRITE, pgnop);
			break;
		default:
			ret = CDbErr::db_unknown_flag(dbp->dbenv, "__ham_c_put", flags);
			break;
		}
		...
		if (*pgnop == PGNO_INVALID && ret == 0) {
			if (flags == DB_CURRENT ||
				...
				ret = ham_overwrite(dbc, data, flags);
			else
				ret = CHashDup::ham_add_dup(dbc, data, flags, pgnop);
		}
	...
	err2:	if ((t_ret = CHashMeta::ham_release_meta(dbc)) != 0 && ret == 0)
			ret = t_ret;
	...
	static int ham_expand_table(DBC *dbc)
	{
		...
		logn = CDbLog2::db_log2(new_bucket);
		...
		if (!new_double || hcp->hdr->spares[logn + 1] != PGNO_INVALID) {
			...
			if ((ret =
				CMpFget::memp_fget(mpf, &pgno, DB_MPOOL_CREATE, &h)) != 0)
				goto err;
			lsn = h->lsn;
		} else {
		...
		if (DBC_LOGGING(dbc)) {
			 ...
			 */
			if ((ret = CHashAuto::ham_metagroup_log(dbp, dbc->txn,
		...
		if (new_double && hcp->hdr->spares[logn + 1] == PGNO_INVALID) {
			...
			if ((ret = CMpFget::memp_fget(mpf, &pgno, DB_MPOOL_CREATE, &h)) != 0)
				goto err;
		...
		ret = CHashPage::ham_split_page(dbc, old_bucket, new_bucket);

	err:	if (got_meta)
			if ((t_ret =
				CMpFput::memp_fput(mpf, mmeta, dirty_meta)) != 0 && ret == 0)
				ret = t_ret;
		if ((t_ret = __TLPUT(dbc, metalock)) != 0 && ret == 0)
				ret = t_ret;
		if (h != NULL)
			if ((t_ret = CMpFput::memp_fput(mpf, h, 0)) != 0 && ret == 0)
				ret = t_ret;
	...
	static int ham_dup_return(DBC *dbc, DBT *val, u_int32_t flags)
	{
		...
		if (flags == DB_GET_BOTH ||
			...
			if (F_ISSET(hcp, H_ISDUP)) {
				...
				CHashDup::ham_dsearch(dbc, val, &off, &cmp, flags);
				...
				if (((HKEYDATA *)hk)->type == H_OFFPAGE) {
					...
					memcpy(&pgno,
						HOFFPAGE_PGNO(hk), sizeof(db_pgno_t));
					if ((ret = CDbOverflow::db_moff(dbp, val,
					...
					cmp = dbp->dup_compare == NULL ?
						CBTreeCompare::bam_defcmp(dbp, &tmp_val, val) :
		 ...
		 * set appropriately.
		 */
		if ((ret = CDbRet::db_ret(dbp, pp, ndx, myval, &dbc->rdata->data,
			&dbc->rdata->ulen)) != 0)
			return (ret);
	...
	static int ham_overwrite(DBC *dbc, DBT *nval, u_int32_t flags)
	{
		...
		if (F_ISSET(hcp, H_ISDUP)) {
			...
			if (F_ISSET(nval, DB_DBT_PARTIAL)) {
				...
				if ((ret =
					ham_dup_return(dbc, &tmp_val, DB_CURRENT)) != 0)
					return (ret);
				...
				if (ISBIG(hcp,
					...
					if ((ret = CHashDup::ham_dup_convert(dbc)) != 0)
						return (ret);
				...
				if (dbp->dup_compare != NULL) {
					...
					if (dbp->dup_compare(
						...
						COsAlloc::os_free(dbenv, newrec);
						return (CDbCam::db_duperr(dbp, flags));
					}
				}
				...
				ret = CHashPage::ham_replpair(dbc, &tmp_val2, 0);
				COsAlloc::os_free(dbenv, newrec);
				...
				if ((ret =
					CHashDup::ham_make_dup(dbp->dbenv, nval, &tmp_val,
			...
			myval = nval;
#endif
		return (CHashPage::ham_replpair(dbc, myval, 0));
	}
	...
	static int ham_lookup(DBC *dbc, const DBT *key, u_int32_t sought, db_lockmode_t mode, db_pgno_t *pgnop)
	{
		 ...
		 */
		if ((ret = CHashPage::ham_item_reset(dbc)) != 0)
			return (ret);
		hcp->seek_size = sought;

		hcp->bucket = ham_call_hash(dbc, (u_int8_t *)key->data, key->size);
		hcp->pgno = BUCKET_TO_PAGE(hcp, hcp->bucket);

		for (;;) {
			*pgnop = PGNO_INVALID;
			if ((ret = CHashPage::ham_item_next(dbc, mode, pgnop)) != 0)
				return (ret);
			...
			case H_OFFPAGE:
				...
				if (tlen == key->size) {
					memcpy(&pgno,
						HOFFPAGE_PGNO(hk), sizeof(db_pgno_t));
					if ((ret = CDbOverflow::db_moff(dbp,
	...
	found_key:			F_SET(hcp, H_OK);
			...
			case H_OFFDUP:
				 ...
				 */
				return (CDbErr::db_pgfmt(dbp->dbenv, PGNO(hcp->page)));
			default:
				return (CDbErr::db_pgfmt(dbp->dbenv, PGNO(hcp->page)));
			}
		}
	...
	static int ham_init_dbt(DB_ENV *dbenv, DBT *dbt, u_int32_t size, void **bufp, u_int32_t *sizep)
	{
		...
		if (*sizep < size) {
			if ((ret = COsAlloc::os_realloc(dbenv, size, bufp)) != 0) {
				*sizep = 0;
				return (ret);
			}
			*sizep = size;
	...
	static int ham_c_update(DBC *dbc, u_int32_t len, int add, int is_dup)
	{
		...
		if (!add) {
			order = 1;
			for (ldbp = DB::dblist_get(dbenv, dbp->adj_fileid);
		...
		if (found != 0 && DBC_LOGGING(dbc)) {
			if ((ret = CHashAuto::ham_curadj_log(dbp, my_txn, &lsn, 0, hcp->pgno,
		...
		for (ldbp = DB::dblist_get(dbenv, dbp->adj_fileid);
			...
			for (cp = TAILQ_FIRST(&ldbp->active_queue); cp != NULL;
				...
				if (cp->internal->pgno == pgno &&
					...
					if (nused >= nalloc) {
						nalloc += 10;
						if ((ret = COsAlloc::os_realloc(dbp->dbenv,
	...
	static int ham_c_writelock(DBC *dbc)
	{
		...
		if ((!LOCK_ISSET(hcp->lock) || hcp->lock_mode != DB_LOCK_WRITE)) {
			tmp_lock = hcp->lock;
			if ((ret = CHashPage::ham_lock_bucket(dbc, DB_LOCK_WRITE)) == 0 &&
