...
{
public:
	static int os_openhandle(DB_ENV *dbenv, const char *name, int flags, int mode, DB_FH **fhpp)
	{
		DB_FH *fhp;
		...
		for (nrepeat = 1; nrepeat < 4; ++nrepeat) {
			...
			switch (ret = COsErrno::os_get_errno()) {
			case EMFILE:
			case ENFILE:
			case ENOSPC:
				/*
				 ...
				 */
				COsSleep::os_sleep(dbenv, nrepeat * 2, 0);
				break;
			case EAGAIN:
			case EBUSY:
			case EINTR:
	...
	static int os_closehandle(DB_ENV *dbenv, DB_FH *fhp)
	{
		int ret;
		...
		if (F_ISSET(fhp, DB_FH_OPENED)) {
			...
			if (ret != 0)
				CDbErr::db_err(dbenv, "close: %s", strerror(ret));
	
			/* Unlink the file if we haven't already done so. */
			if (F_ISSET(fhp, DB_FH_UNLINK)) {
				(void)COsUnlink::os_unlink(dbenv, fhp->name);
				COsAlloc::os_free(dbenv, fhp->name);
			}
		}
	
		COsAlloc::os_free(dbenv, fhp);
