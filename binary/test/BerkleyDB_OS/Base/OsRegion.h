...
// extracted from os_region.c


class COsRegion
{
public:
	static int os_r_attach(DB_ENV *dbenv, REGINFO *infop, REGION *rp)
	{
		int ret;
...
#define	OS_VMROUNDOFF(i) {						\
	...
	(i) -= (i) % OS_VMPAGESIZE;					\
		...
		if (rp->size > DB_REGIONSIZE_MAX) {
			CDbErr::db_err(dbenv, "region size %lu is too large; maximum is %lu",
	    ...
	    "application may not specify both DB_PRIVATE and DB_THREAD");
			...
			if ((ret = COsMap::os_r_sysattach(dbenv, infop, rp)) != 0)
				return (ret);
		}
	...
	static int os_r_detach(DB_ENV *dbenv, REGINFO *infop, int destroy)
	{
		REGION *rp;
		...
		if (F_ISSET(dbenv, DB_ENV_PRIVATE)) {
			COsAlloc::os_free(dbenv, infop->addr);
			return (0);
		}
		...
		return (COsMap::os_r_sysdetach(dbenv, infop, destroy));
	}
};
