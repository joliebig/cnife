...
{
  public:
	  LatexGenerator() : OutputGenerator()
	  {
	...
	void printDoc(DocNode *n,const char *langExt)
	{
	...
	void endIndexKey() { }
	void startIndexValue(bool hasBrief)
	{
		t << " ";
		if (hasBrief) t << "(";
	}
	void endIndexValue(const char *name,bool hasBrief)
	{
		if (hasBrief) t << ")";
		t << "}{\\pageref{" << name << "}}{}" << endl;
	}
	...
	{
		filterLatexString(t,text,insideTabbing,FALSE);
	}
	...
	void startAnonTypeScope(int indent)
	{
		if (indent==0)
		{
	...
	void endAnonTypeScope(int indent)
	{
		if (indent==0)
		{
			t << endl << "\\end{tabbing}";
	...
	void startMemberItem(int annoType) 
	{ 
	...
	void startMemberGroupHeader(bool hasHeader)
	{
	...
	void endMemberGroup(bool hasHeader)
	{
	...
	{ 
		...
		if (usePDFLatex && pdfHyperlinks)
		{
			if (fileName)
			{
				t << "\\hypertarget{" << stripPath(fileName) << "_" << name << "}{}" << endl;
			}
			else
			{
	...
	void writeLineNumber(const char *ref,const char *fileName,const char *anchor,int l)
	{
		if (m_prettyCode)
		{
			QCString lineNumber;
	...
	void startMemberDoc(const char *clname,
		const char *memname,
		const char *,
		const char *title)
	{ 
	...
	void startDoxyAnchor(const char *fName,const char *,
	...
	void startCodeAnchor(const char *name) 
	{
		static bool usePDFLatex = Config_getBool("USE_PDFLATEX");
		static bool pdfHyperlinks = Config_getBool("PDF_HYPERLINKS");
	...
	void startSimpleSect(SectionTypes,const char *file,
    ...
    void endDescForItem()       {}
	void startSection(const char *lab,const char *,SectionInfo::SectionType type)
	{
		static bool pdfHyperlinks = Config_getBool("PDF_HYPERLINKS");
		static bool usePDFLatex   = Config_getBool("USE_PDFLATEX");
		if (usePDFLatex && pdfHyperlinks)
	...
	void endSection(const char *lab,SectionInfo::SectionType)
	{
		t << "}\\label{" << lab << "}" << endl;
	}
	...
	void endClassDiagram(const ClassDiagram &d,
	...
	void endPageRef(const char *clname,const char *anchor)
	{
		t << "}{";
		if (clname) t << clname; 
		if (anchor) t << "_" << anchor;
	...
	void startFontClass(const char *name)
	{
		if (!m_prettyCode) return;
		t << "\\textcolor{" << name << "}{";
	}
