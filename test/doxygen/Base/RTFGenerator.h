...
class RTFGenerator : public OutputGenerator
{
  public:
	  ...
	  static void writeStyleSheetFile(QFile &file)
	  {
		  QTextStream t(&file);
		...
		const char *path,const char *name)
	{
		DBG_RTF(t << "{\\comment (writeStartAnnoItem)}" << endl)
			t << "{\\b ";
		if (path) docify(path);
		if (file && Config_getBool("RTF_HYPERLINKS"))
		{
			t << "{\\field {\\*\\fldinst { HYPERLINK  \\\\l \"";
			t << rtfFormatBmkStr(file);
			t << "\" }{}";
			t << "}{\\fldrslt {\\cs37\\ul\\cf2 ";
	...
	void startSimpleSect(SectionTypes,const char *file,const char *anchor,const char *title)
	{
	...
	void startSection(const char *,const char *title,SectionInfo::SectionType type)
	{
...
static bool PreProcessFile(QDir &d,QCString &infName, QTextStream &t, bool bIncludeHeader)
{
	QFile f(infName);
